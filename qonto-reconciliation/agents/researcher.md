...
2. **Otherwise, work through the channels in `channel-taxonomy.md`** (bundled in the parent skill's `reference/` directory — read it if you haven't already this session) in order of cost: direct email attachment, hosted link, web portal, local deposit folder, request-and-wait, and only fall back to open web search if the supplier/portal itself is unknown.
